...
class ShopCart(models.Model):
    ...
    variant = models.ForeignKey(
        Variants, on_delete=models.SET_NULL, null=True, blank=True)  # relation with variant
...
class Order(models.Model):
    ...
    transporter = models.ForeignKey(
        Transporter, on_delete=models.SET_NULL, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if created and self.transporter:
            TransporterOrder.objects.filter(order=self).update(
            ...
            try:
                ...
                for pr in order_items:
                    tr_order = TransporterOrder(
                        product=pr.product, quantity=pr.quantity, order=self, transporter=self.transporter)
                    tr_order.save()
...
class TransporterOrder(models.Model):
    ...
    transporter = models.ForeignKey(
        Transporter, on_delete=models.SET_NULL, null=True)
...
class OrderItem(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.SET_NULL, null=True, blank=True, related_name='items')
    variant = models.ForeignKey(
        Variants, on_delete=models.SET_NULL, null=True, blank=True, related_name='items')
    vendor = models.ForeignKey(
        Vendor, related_name='items', on_delete=models.CASCADE
    )
    vendor_paid = models.BooleanField(default=False)
    price = models.DecimalField(max_digits=8, decimal_places=2)
    price_no_vat = models.DecimalField(
